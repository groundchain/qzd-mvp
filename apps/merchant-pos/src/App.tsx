...
export default function App(): JSX.Element {
  ...
  const navLinks = [
    { href: '#auth-section', label: 'Merchant access' },
    { href: '#account-section', label: 'Account monitor' },
    { href: '#invoice-section', label: 'Invoice details' },
    { href: '#history-section', label: 'Invoice history' },
  ];
    ...
    <div className="app-shell">
      ...
      <header className="app-header">
        <div className="container">
          <div>
            <h1 className="app-title">QZD Merchant POS</h1>
            <p className="app-subtitle">Generate QR invoices, confirm wallet payments, and export receipts.</p>
          </div>
          <div className="session-meta">
            <span className="session-pill" data-status={isAuthenticated ? 'active' : 'inactive'}>
              {isAuthenticated ? 'Merchant signed in' : 'Sign in required'}
            </span>
            <span>
      ...
      <nav className="app-nav" aria-label="Merchant POS sections">
        <div className="container">
          <ul>
            {navLinks.map((link) => (
              <li key={link.href}>
      ...
      <main id="main-content" className="app-main">
        <div className="container">
          <section className="section-card" aria-labelledby="auth-section-title" id="auth-section">
            <header>
              <h2 id="auth-section-title">Merchant access</h2>
              <p>Create or sign in to a merchant account to monitor payments.</p>
            </header>
            {isAuthenticated ? (
              ...
              <div className="auth-layout">
                ...
                <form onSubmit={handleLogin} className="auth-form">
                  ...
                  <button type="submit" disabled={authStatus === 'pending'}>
                    {authStatus === 'pending' ? 'Checking...' : 'Sign in'}
                  </button>
                </form>
              </div>
            )}
          ...
          <section className="section-card" aria-labelledby="account-section-title" id="account-section">
            <header>
              <h2 id="account-section-title">Account monitor</h2>
              <p>Select a settlement account to watch for matching transactions.</p>
            </header>
            {!isAuthenticated ? (
              <EmptyState
                title="Sign in to monitor accounts"
                description="Authenticate above to start watching settlement activity."
              />
            ) : (
              <>
                ...
                {hasAccount ? (
                  <p>
                    Monitoring account <strong>{accountId}</strong>. Polling for transactions every {POLL_INTERVAL_MS / 1000}{' '}
                    seconds.
                  </p>
                ) : (
                  <EmptyState
                    title="No account selected"
                    description="Enter an account ID to generate invoices and match incoming payments."
                  />
                )}
              </>
            )}
          ...
          <section className="section-card" aria-labelledby="invoice-section-title" id="invoice-section">
            <header>
              <h2 id="invoice-section-title">Invoice details</h2>
              <p>Generate QR invoices and share the memo with customers.</p>
            </header>
            {!isAuthenticated ? (
              <EmptyState
                title="Sign in to create invoices"
                description="Authenticate above before generating payment requests."
              />
            ) : !hasAccount ? (
              <EmptyState
                title="Select an account"
                description="Choose a settlement account before issuing invoices."
              />
            ) : (
              <>
                ...
                <p>
                  Share the QR code and invoice memo with the customer. Wallet users should paste the memo into the transfer form
                  so the POS can auto-confirm payment.
                </p>
              </>
            )}
          </section>

          <section className="section-card" aria-labelledby="history-section-title" id="history-section">
            <header>
              <h2 id="history-section-title">Invoice history</h2>
              <p>Track invoice status and export receipts for paid transactions.</p>
            </header>
            {!isAuthenticated ? (
              <EmptyState
                title="Sign in to view invoices"
                description="Authenticate above to review generated invoices."
              />
            ) : invoices.length === 0 ? (
              <EmptyState
                title="No invoices yet"
                description="Create an invoice to start tracking payment status."
              />
            ) : (
                  ...
                  <li key={invoice.id} className="invoice-card">
                    ...
                    <div className="qr-wrapper">
                      ...
                      <textarea
                        readOnly
                        value={invoice.memo}
                        aria-label={`Memo for invoice ${invoice.reference}`}
                      />
                    </div>
      ...
      <footer className="footer">
        <div className="container">Merchant tooling for demos, QA, and in-store pilots.</div>
      </footer>
    </div>
  );
}
