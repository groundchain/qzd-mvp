...
function createSignaturePayload(method: string, pathWithQuery: string, idempotencyKey: string, nonce: string, serializedBody: string): Uint8Array {
...
export function createSignedFetch(signingKeyHex?: string, baseFetch: FetchImplementation = fetch): FetchImplementation {
